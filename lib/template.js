...
//                <td>${inputs[i].id}</td>
    ...
    showTotalrecodesTable:function(inputs){
        ...
        while(i < inputs.length) {
          ...
          <tr ${not_comp_bg_color}>
            <td style="text-align:center; background-color:#999" ><a href="/showNupdatetable?date=${inputs[i].daykey}">${inputs[i].daykey}</a></td>
            <td style="text-align:center;">${inputs[i].sleepstart} </td>
            <td style="text-align:center;">${inputs[i].sleepend}   </td>
            <td style="text-align:center;">${minute_to_HM(inputs[i].sleep)}</td>
            <td style="text-align:center;">${nicetime_string}</td>
            <td style="text-align:center;">${(inputs[i].nicetime / 240 * 100).toFixed(2)}%</td>
          </tr>
          `
          i++;
        }
