...
*/
    ...
    update_process:function(request, response) {
        ...
        request.on('end', function(){
            ...
            var id = post.id;
            var date_only_number = post.date;
            db.query(`UPDATE day` + date_only_number + ` SET category=?, doing=?, starth=?, startm=?, endh=?, endm=?, consume=?, except_check=? WHERE id=?`, [post.category, post.doing, post.starth, post.startm, post.endh, post.endm, post.consume, post.except_check, post.id], 
    ...
    confirm_process:function(request, response) {
        ...
        request.on('end', function(){
            ...
            var id = post.id;
            var date_only_number = post.date; 
            var date_only_number__year = post.date[0] + post.date[1];
            var date_only_number__month = post.date[2] + post.date[3];
            var date_only_number__day = post.date[4] + post.date[5];
